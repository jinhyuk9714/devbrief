...
백엔드에서는 source별로 성공, 데모, 대체 데이터, 실패 상태를 기록합니다. 네트워크가 꺼져 있거나 RSS 파싱이 실패해도 전체 수집이 멈추지 않고 source별 결과를 관리자 화면에 남깁니다. Redis가 있으면 ingestion job을 distributed gate로 보호하고, Redis가 없으면 local lock으로 데모 동작을 유지합니다. OpenAI 키가 있으면 품질 강화 프롬프트와 응답 검증을 거쳐 실제 한국어 요약을 만들고, 없거나 실패하면 source 이름, 원문 제목, excerpt를 반영한 fallback 요약을 생성합니다.
...
OpenAI provider는 abstraction 뒤에 있고, 키가 없거나 응답 파싱/검증이 실패하면 deterministic fallback provider로 내려갑니다. OpenAI 응답은 한국어 여부, lead source/title 원문 유지, 일반론 여부, 실행 가능한 action item 형태를 확인합니다. fallback도 단순 카테고리 템플릿이 아니라 lead article의 source, title, excerpt를 섞어서 브리핑별 문장이 달라지게 했습니다.
...
첫 번째는 실제 운영 RSS source를 늘리고 parser fixture를 더 쌓는 것입니다. 두 번째는 BM25/IDF 그룹핑 위에 embedding 기반 유사도 그룹핑을 선택적으로 올리는 것입니다. 세 번째는 OpenAI 요약 품질을 실제 사용자 피드백이나 평가셋으로 더 정량화하는 것입니다.

...
OpenAI provider는 abstraction 뒤에 있고, 키가 없거나 응답 파싱/검증이 실패하면 deterministic fallback provider로 내려갑니다. OpenAI 응답은 한국어 여부, lead source/title 원문 유지, 일반론 여부, 실행 가능한 action item 형태를 확인합니다. 이 검증은 대표 기사 묶음 fixture로 회귀 테스트합니다. fallback도 단순 카테고리 템플릿이 아니라 lead article의 source, title, excerpt를 섞어서 브리핑별 문장이 달라지게 했습니다.

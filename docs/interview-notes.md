...
이 프로젝트는 단순 뉴스 목록이나 CRUD 앱이 아니라 외부 데이터 수집부터 운영 상태까지 이어지는 풀스택 파이프라인입니다. 핵심 흐름은 `source 수집 -> 중복 제거 -> 휴리스틱 그룹핑/점수화 -> 한국어 요약 -> 캐시/상태 표시`입니다.
...
현재는 임베딩 기반 semantic clustering이 아니라 시그널 기반 휴리스틱 그룹핑입니다. 제목과 excerpt에서 MCP, browser trace replay, GitHub Actions provenance 같은 강한 신호를 잡고, 나머지는 token similarity로 묶습니다. README에도 이 구현 수준을 투명하게 적었습니다. 다음 단계로는 embedding similarity 또는 BM25 기반 유사도 검색을 붙일 수 있습니다.
...
현재 공개 API는 무료 Render Docker web service에서 돌아가고 있어 H2 memory storage를 사용합니다. Render의 managed PostgreSQL/Key Value Blueprint는 결제 정보가 필요해서 README에 제약을 명시했고, 실제 full setup용 `render.yaml`은 함께 커밋해두었습니다.
...
첫 번째는 실제 운영 RSS source를 늘리고 parser fixture를 더 쌓는 것입니다. 두 번째는 휴리스틱 그룹핑을 embedding 기반 유사도 그룹핑으로 올리는 것입니다. 세 번째는 OpenAI 요약 결과의 JSON schema 검증과 출처 근거 표시를 더 강화하는 것입니다.
